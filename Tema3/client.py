...
class FTPClient:
    ...
    def _pick_server_file(self):
        """Helper: list server files and return the selected filename, or None."""
    ...
    def read_file(self):
        ...
        filename = self._pick_server_file()
        if not filename:
            return
    ...
    def edit_file(self):
        ...
        response = self.send_command({'command': 'edit_file', 'filename': filename, 'content': new_content})
    ...
    def see_file_operation_history(self):
        ...
        if response['status'] == 'success':
            ...
            if history:
                ...
                print(f"✗ No history recorded for '{filename}'")
        else:
            print(f"✗ {response['message']}")
